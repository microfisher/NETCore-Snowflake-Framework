...
namespace Snowflake.Data
{
    ...
    public class Repository<TEntity, TKey> : IRepository<TEntity, TKey>
    {
        ...
        public virtual async Task<TEntity> GetSingleAsync(TKey id, bool isForUpdate = false)
        {
        ...
        {
            var sql = $"SHOW TABLES LIKE '{typeof(TEntity).Name}'";
            var data = await UnitOfWork.Connection.ExecuteScalarAsync(sql);
            return data != null && data.ToString().ToLower().Equals(typeof(TEntity).Name.ToLower());
        }
        ...
        public virtual async Task<List<TEntity>> GetPageListAsync(long pageIndex, long pageSize)
        {
